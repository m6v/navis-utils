...
└── var/
    └── lib/
        └── vlab-srorage/               # Корневая директория проекта контейнера
...
- `/var/lib/machines/vlab-servers/upper/entrypoint.sh` — скрипт контейнера (назначает IP 10.0.0.254 интерфейсу `host0`, запускает `chronyd -d -x` и через `chronyc settime` устанавливает смещенное время);
