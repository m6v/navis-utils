...
└── var/
    └── lib/
        └── machines/
            └── vlab-servers/                # Корневая директория проекта контейнера
...
- `/etc/systemd/nspawn/vlab-servers.nspawn` — связывает контейнер с мостом `virbr0` и фиксирует имя интерфейса;
- `/usr/local/bin/timemachine.sh` — скрипт хоста который добавляет к смещению времени время простоя хоста;
- `/var/lib/machines/vlab-servers/upper/entrypoint.sh` — скрипт контейнера (назначает IP 10.0.0.254 интерфейсу `host0`, запускает `chronyd -d -x` и через `chronyc settime` устанавливает время в соответствии с faketime);
- `/var/lib/machines/vlab-servers/upper/etc/chrony/chrony.conf` — конфиг chrony внутри контейнера (включает режим `manual`, `local stratum 10`, открывает доступ для сети 10.0.0.0/24 и делает привязку к bindaddress 10.0.0.254).
