...
2. Изменить порядок работы с дистрибутивами ПО и обновлениями Astra Linux. Теперь из получаем с веб-сервера, а не виртуальных DVD!
2. Удалять только пользователей группы learners после предварительной проверки отсутствия пула дисков
3. В плейбуке создания пользователя сделать выход, если пользователь существует, вместо проверки соответствующей переменной в каждой задаче. И наоборот в задаче удаления пользователя!
4. Решить задачу манипулирования временем не изнутри виртуальной машины сервера СЗИ, а снаружи (наиболее перспективный способ - использовать qemu-guest-agent)
5. Продумать необходимость автоматического запуска ips-master и ips-slave при запуске arm-ips (использовать хук qemu)
6. Разобраться для чего в Астре нужен пакет astra-kvm-secure, который по дефолту не ставится и виртуализация работает без него
