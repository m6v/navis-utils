1. Доделать роль для установки на АРМ инструктора панели с веб-интерфейсом виртуальных машин обучаемых (копирование deb-пакетов для novnc, установка novnc, копирование сервера, файла токенов, службы запуска сервера)
2. Уточнить необходимость флага -x в chronyd -d -x (полностью запрещает chronyd использовать метод step (резкий перевод стрелок часов назад или вперед), но нам наоборот нужно делать резкий перевод) 
3. Изменить порядок работы с дистрибутивами ПО и обновлениями Astra Linux. Теперь из получаем с веб-сервера, а не виртуальных DVD!
4. Удалять только пользователей группы learners после предварительной проверки отсутствия пула дисков
5. В плейбуке создания пользователя сделать выход, если пользователь существует, вместо проверки соответствующей переменной в каждой задаче. И наоборот в задаче удаления пользователя!
6. Решить задачу манипулирования временем не изнутри виртуальной машины сервера СЗИ, а снаружи (наиболее перспективный способ - использовать qemu-guest-agent)
7. Продумать необходимость автоматического запуска ips-master и ips-slave при запуске arm-ips (использовать хук qemu)
8. Разобраться для чего в Астре нужен пакет astra-kvm-secure, который по дефолту не ставится и виртуализация работает без него
