1. В начале скрипта vlab.sh cделать запрос имени пользователя для ssh-подключения, предлагая по дефолту имя из настроек (administrator)
2. В плейбуке создания пользователя сделать выход, если пользователь существует, вместо проверки соответствующей переменной в каждой задаче. И наоборот в задаче удаления пользователя!
2. В arm-ips изменить ти диска на sda
3. Переделать передачу имен хостов в виде списка и разрешать выбор нескольких хостов (если получится добавить кнопку "Выбрать все" и "Очистить")
